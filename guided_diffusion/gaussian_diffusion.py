...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        if use_shortcut:
            steps = int(self.num_timesteps / 1000 * shortcut_start_time)
            pbar = tqdm(list(range(steps))[::-1])
        else:
            pbar = tqdm(list(range(self.num_timesteps))[::-1])
        # pbar = tqdm(list(range(400))[::-1])
        for idx in pbar:
            # print(f'idx: {idx}')
...
class DDPM(SpacedDiffusion):
    def p_sample(self, model, x, t):
        ...
        func = lambda x: self.p_mean_variance(model, x, t)["pred_xstart"]

        return {'sample': sample, 'pred_xstart': out['pred_xstart'], "sigma_t": sample_std, 'mean': sample_mean,'func': func}
...
class DDIM(SpacedDiffusion):
    ...
    def p_sample(self, model, x, t):
        ...
        return {'mean': sample_mean, "sample": sample, "sigma_t": sigma, "pred_xstart": out["pred_xstart"], "func": func}
