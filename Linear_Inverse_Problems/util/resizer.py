...
class Resizer(nn.Module):
    def __init__(self, in_shape, scale_factor=None, output_shape=None, kernel=None, antialiasing=True):
        ...
        for dim in self.sorted_dims:
            ...
            # print(f'field_of_view:{torch.tensor(field_of_view.T.astype(np.int32), dtype=torch.long).shape}')
            # print(f'weights:{torch.reshape(weights, list(weights.shape) + (len(scale_factor) - 1) * [1]).shape}')
